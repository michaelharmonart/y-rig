...
from mgear.core import applyop, attribute, curve, fcurve, node, primitive, string, transform, vector
...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        self.ik0_out = primitive.addTransform(self.ik0_ctl, self.getName("ik0_out"), t)

        # pelvis
        self.length0 = vector.getDistance(self.guide.pos["root"], self.guide.pos["spineBase"])
        vec_po = datatypes.Vector(0, 0.5 * self.length0 * -1, 0)
        self.pelvis_npo = primitive.addTransform(self.ik0_ctl, self.getName("pelvis_npo"), t)
        ...
        self.pelvis_lvl = primitive.addTransform(
            ...
            transform.setMatrixPosition(t, self.guide.pos["root"]),
        )
        self.jnt_pos.append(
            {
                "obj": self.pelvis_lvl,
        ...
        t = transform.setMatrixPosition(t, self.guide.pos["spineTop"])
        if self.settings["autoBend"]:
            ...
            attribute.setKeyableAttributes(self.autoBend_ctl, ["tx", "ty", "tz", "ry"])
            ...
            ik_t = transform.setMatrixPosition(ik_t, self.guide.pos["spineTop"])
            ...
            t = transform.setMatrixPosition(t, self.guide.pos["spineTop"])
            ik_t = transform.setMatrixPosition(ik_t, self.guide.pos["spineTop"])
            self.ik1_npo = primitive.addTransform(self.root, self.getName("ik1_npo"), ik_t)
        ...
        self.ik1_out = primitive.addTransform(self.ik1_ctl, self.getName("ik1_out"), t)
        ...
        if self.settings["centralTangent"]:
            ...
            self.tan0_npo = primitive.addTransform(self.ik0_out, self.getName("tan0_npo"), t)

            self.tan0_off = primitive.addTransform(self.tan0_npo, self.getName("tan0_off"), t)
            ...
            self.tan1_npo = primitive.addTransform(self.ik1_out, self.getName("tan1_npo"), t)

            self.tan1_off = primitive.addTransform(self.tan1_npo, self.getName("tan1_off"), t)
            ...
            vec_pos = vector.linearlyInterpolate(
                self.guide.pos["spineBase"], self.guide.pos["spineTop"], 0.5
            )
            t = transform.setMatrixPosition(t, vec_pos)

            self.tan_npo = primitive.addTransform(self.tan0_npo, self.getName("tan_npo"), t)
        ...
        t = transform.setMatrixPosition(t, self.guide.pos["chest"])
        self.chest_npo = primitive.addTransform(
    ...
    def addOperators(self):
        ...
        # Tangent position ---------------------------------
        # common part
        d = vector.getDistance(self.guide.pos["spineBase"], self.guide.pos["spineTop"])
        ...
        # Curves -------------------------------------------
        op = applyop.gear_curveslide2_op(self.slv_ref_crv, self.mst_crv, 0, 1.5, 0.5, 0.5)
        ...
        pm.connectAttr(arclen_node.attr("arcLength"), muldiv_node.attr("input1X"))
        ...
        for i in range(self.settings["division"]):
            ...
            # Orientation Lock
            if i == 0:
                dm_node = node.createDecomposeMatrixNode(self.ik0_out + ".worldMatrix")
                ...
                dm_node = node.createDecomposeMatrixNode(self.ik1_out + ".worldMatrix")

                mulmat_node2 = applyop.gear_mulmatrix_op(
                    self.spineTwistChain[-1].attr("worldMatrix"),
                    self.div_cns[i].attr("parentInverseMatrix"),
                )
                dm_node2 = node.createDecomposeMatrixNode(mulmat_node2 + ".output")
        ...
        t = transform.setMatrixPosition(transform.getTransform(self.cnx1), self.guide.pos["chest"])
